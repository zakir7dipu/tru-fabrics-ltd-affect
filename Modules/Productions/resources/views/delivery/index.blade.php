                            ...
                            <div class="row mb-2">
                                ...
                                <div class="col-xl-4">
                                    <div class="text-xl-end mt-xl-0 mt-2">
                                        @if (isOptionPermitted('finished-good-delivery'))
                                            <a href="{{ route('finished-goods-deliveries.create') }}"
                                               class="btn btn-info mb-2 me-2"
                                               data-toggle="tooltip" title="Add New"> <i
                                                    class="mdi mdi-plus
                                           me-1"></i>{{ translate('FG Delivery') }}</a>
                                        @endif
                                    </div>
                                </div>
                            </div>
...
@section('javascript')
    ...
    <script>
        ...
        function showDetails(id) {
            $('#showUserDetailsModal .modal-title').html('Finished Goods Delivery Details');
            $('#dataBody').empty().load('{{ url(Request()->route()->getPrefix().'/finished-goods-deliveries') }}/' + id);
            $('#showUserDetailsModal').modal('show');
        }
    </script>
@endsection
