...
<div class="row">
            ...
            <div class="form-line">
                ...
                <select name="charge_id"
                    ...
                    @foreach($charges as $data)
                        <option
                            value="{{$data->id}}" {{$workOrderCharge->charge_id===$data->id?'selected':''}}>{{$data->charge_name}}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="form-line">
                ...
                <select class="form-control select2bs4 currency"
                    ...
                    @foreach($currenciesModel as $data)
                        <option
                            value="{{$data->id}}" {{$workOrderCharge->currency_id===$data->id?'selected':''}}>{{$data->short_code}}
                            ({{$data->symbol}})
                        </option>
                    @endforeach
                </select>
    ...
    <div class="col-md-3">
        <div class="form-group">
            <div class="form-line">
                {!!  Form::label('currency_convert_rate', 'Currency Convert Rate', ['class' => 'col-form-label']) !!}
                <span class="text-danger">*</span>
                <input type="number" name="currency_convert_rate"
                       ...
                       required
                       step="any"
                       value="{{$workOrderCharge->currency_convert_rate}}"
                >
            </div>
        </div>
    </div>
