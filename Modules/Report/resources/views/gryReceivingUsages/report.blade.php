    ...
    @if($inventories)
        @foreach($inventories as $value)
            ...
            <tr>
                ...
                <td class="text-center">{{$value->requisitionDeliveryItems->sum('issued_qty')}}</td>
                <td>{{$value->product->productUnit->unit_name}}</td>
                <td class="text-right">{{systemMoneyFormat($value->grnItem->unit_price,'$')}}</td>
                <td class="text-right">{{systemMoneyFormat($value->requisitionDeliveryItems->sum('issued_qty')*$value->grnItem->unit_price, '$')}}</td>
                <td class="text-left">{{$value->requisitionDeliveryItems->unique('requisitionDelivery.requisition.workOrderItem.workOrder.work_order_no')->pluck('requisitionDelivery.requisition.workOrderItem.workOrder.work_order_no')->implode(', ')}}</td>
