<div class="row">
    ...
    <div class="col-md-3">
        <div class="form-group">
            <div class="form-line">
                {!!  Form::label('unit_id', 'Unit', array('class' => 'col-form-label')) !!}
                {!! Form::Select('unit_id',$units,Request::old('unit_id'),['id'=>'unit_id', 'class'=>'form-control select2']) !!}
    ...
    <div class="col-md-3">
        <div class="form-group">
            <div class="form-line">
                {!!  Form::label('product_group_id', 'ProductGroup', array('class' => 'col-form-label')) !!}
                {!! Form::Select('product_group_id',$productGroups,Request::old('product_group_id'),['id'=>'product_group_id', 'class'=>'form-control select2']) !!}
    ...
    <div class="col-md-3">
        <div class="form-group">
            <div class="form-line">
                {!!  Form::label('category_id', 'Category', array('class' => 'col-form-label')) !!}
                {!! Form::Select('category_id',$categories,Request::old('category_id'),['id'=>'category_id', 'class'=>'form-control select2']) !!}
    ...
    <div class="col-md-2">
        <div class="form-group">
            <div class="form-line">
                ...
                {!! Form::Select('mode_of_purchase',array('import'=>'Import','native'=>'Native'),Request::old('mode_of_purchase'),['id'=>'mode_of_purchase', 'class'=>'form-control select2']) !!}
    ...
    <div class="col-md-2">
        <div class="form-group">
            <div class="form-line">
                {!!  Form::label('is_finished_goods', 'Is Finished Good?', array('class' => 'col-form-label')) !!}
                {!! Form::Select('is_finished_goods',array('no'=>'No','yes'=>'Yes'),Request::old('is_finished_goods'),['id'=>'is_finished_goods', 'class'=>'form-control select2']) !!}
                {!! $errors->first('is_finished_goods') !!}
            </div>
        </div>
    </div>
    <div class="col-md-2">
        ...
        <select name="productAttributes[]" id="attributes"
            ...
            @if(isset($attributes[0]))
                @foreach($attributes as $key => $attribute)
                    <option
                        value="{{ $attribute->id }}" {{ in_array($attribute->id, $productAttributes) ? 'selected' : '' }}>{{ $attribute->name }}</attribute>
                @endforeach
            @endif
        </select>
    </div>
    ...
    <div class="col-md-12 mt-3">
        <div class="row">
            @if(isset($attributes[0]))
                @foreach($attributes as $key => $attribute)
                    <div class="col-md-3 mt-1 attributes attribute-{{ $attribute->id }}"
                         style="display: {{ in_array($attribute->id, $productAttributes) ? 'block' : 'none' }}">
                        ...
                        <div class="input-group input-group-md">
                            <select name="attribute_option_id[{{ $attribute->id }}]" id="attribute-{{ $attribute->id }}"
                                    class="form-control rounded attribute-options">
                                <option value="0">Not Required</option>
                                @if(isset($attribute->options[0]))
                                    @foreach($attribute->options()->orderBy('name','asc')->get() as $key => $option)
                                        <option
                                            value="{{ $option->id }}" {{ $product->attributes->where('attribute_option_id', $option->id)->count() > 0 ? 'selected' : '' }}>{{ $option->name }}</option>
                                    @endforeach
                                @endif
                            </select>
                        </div>
